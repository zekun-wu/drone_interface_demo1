...
function App() {
  ...
  const generateDroneDataAndVideoFiles = () => {
    const normalSceneFiles = [];
    for (let i = 1; i <= 3; i++) {
      const data = `${process.env.PUBLIC_URL}/data/normal_scenes/${i}/data.json`;
      const video = `${process.env.PUBLIC_URL}/assets/normal_scenes/${i}.mp4`;
      normalSceneFiles.push({ data, video });
    }
    ...
    const droneVideoFiles = droneFiles.map(file => file.video);

    return { dataFiles: droneDataFiles, videoFiles: droneVideoFiles };
  };

  const { dataFiles: initialDroneDataFiles, videoFiles: initialDroneVideoFiles } = generateDroneDataAndVideoFiles();
  const [droneDataFiles, setDroneDataFiles] = useState(initialDroneDataFiles);
  const [droneVideoFiles, setDroneVideoFiles] = useState(initialDroneVideoFiles);
  ...
  useEffect(() => {
    if (prevScene !== currentSceneHighlight.scene) {
      const { dataFiles, videoFiles } = generateDroneDataAndVideoFiles();
      setDroneDataFiles(dataFiles);
      setDroneVideoFiles(videoFiles);
      ...
      <div className="app_container">
        {(scenehighlightArray[currentIndex] === 'questionnaire' ) ? (
          ...
          <MemoizedDroneMonitor
            ...
            highlight={currentSceneHighlight.highlight}
            droneDataFiles={droneDataFiles}
            droneVideoFiles={droneVideoFiles}
            droneData={droneData}
